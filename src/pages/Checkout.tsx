...
const resolveRoomLocationValue = (selection: string, details: string) => {
  if (selection === 'Other') {
...
const normalizeRoomState = (locationValue?: string | null, detailsValue?: string | null) => {
  if (locationValue && locationValue.startsWith('Other - ')) {
    const extracted = locationValue.replace(/^Other - /i, '').trim();
    return { location: 'Other', details: detailsValue || extracted };
  }
  return { location: locationValue || '', details: detailsValue || '' };
};
...
export default function Checkout() {
  ...
  const normalizedCart = normalizeRoomState(cart?.room_location, cart?.room_details);
  const [roomLocation, setRoomLocation] = useState<string>(normalizedCart.location);
  const [roomDetails, setRoomDetails] = useState<string>(normalizedCart.details);
  ...
  useEffect(() => {
    if (cart) {
      const normalized = normalizeRoomState(cart.room_location, cart.room_details);
      setRoomLocation(normalized.location);
      setRoomDetails(normalized.details);
  ...
  const handleRoomChange = (value: string) => {
    ...
    if (value !== 'Other') {
      ...
      const resolved = resolveRoomLocationValue(value, roomDetails);
      updateCartDetails({ room_location: resolved, room_details: roomDetails || null });
    }
  };
  ...
  const handleRoomDetailsChange = (value: string) => {
    ...
    if (roomLocation === 'Other') {
      const resolved = resolveRoomLocationValue(roomLocation, value);
      updateCartDetails({ room_details: value, room_location: resolved });
    }
  };
  ...
  const createOrder = async () => {
    ...
    const resolvedRoomLocation = showLocationQuestions ? resolveRoomLocationValue(roomLocation, trimmedDetails || '') : null;
    ...
    try {
      ...
      await updateCartDetails(
        showLocationQuestions
          ? {
              room_location: resolvedRoomLocation,
              ...
              {roomLocation === 'Other' && (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Example: Ground floor next to the entrance"
                  />
                </div>
              )}
